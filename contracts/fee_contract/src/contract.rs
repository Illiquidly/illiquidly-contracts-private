...
use cosmwasm_std::{
    to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdResult, WasmQuery, QueryRequest
};


use fee_contract_export::msg::{ExecuteMsg, InstantiateMsg, QueryMsg, FeeResponse, into_cosmos_msg};
...
pub fn query_fee_for(
    ...
    let trade_info = deps.querier.query(&QueryRequest::Wasm(
        WasmQuery::Smart {
            contract_addr: contract_info.p2p_contract.to_string(),
            msg: to_binary(&P2PQueryMsg::TradeInfo{
                trade_id
            })?,
    }))?;

    if let Some(counter_id) = counter_id{
        let counter_info = deps.querier.query(&QueryRequest::Wasm(
            WasmQuery::Smart {
                contract_addr: contract_info.p2p_contract.to_string(),
...
pub mod tests {
    ...
    fn pay_fee_helper(deps: DepsMut,
        trader: &str,
        trade_id:u64
    )-> Result<Response, ContractError> {
