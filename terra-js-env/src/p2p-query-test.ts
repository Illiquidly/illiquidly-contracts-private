...
async function main(){
	...
	while(c){
		...
		if(response && response.counter_trades && response.counter_trades.length > 0){
			start_after = response.counter_trades[response.counter_trades.length-1].counter_id;
		}else{
			c = false;
		}
	}
	...
	while(c){
		...
		if(response && response.counter_trades && response.counter_trades.length > 0){
			start_after_any = {
				trade_id: response.counter_trades[response.counter_trades.length-1].trade_id,
				counter_id: response.counter_trades[response.counter_trades.length-1].counter_id,
			}
		}else{
			c = false;
		}
