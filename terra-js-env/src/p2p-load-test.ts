...
async function main(){
	...
	let response: any; 
	console.log("p2p address", p2p.address)
	...
	// Now we create 1000 trades in a signle transaction
	let txArray: MsgExecuteContract[] = [];
	for(let i=0;i<1000;i++){
		txArray.push(new MsgExecuteContract(
	...
	response = await p2p.execute.confirm_counter_trade({
		trade_id: trade_id,
		counter_id: 150
	});
}
