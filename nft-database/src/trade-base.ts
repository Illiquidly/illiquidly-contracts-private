...
async function getAllTrades(
  ...
  const tradeInfoResponse: any = await lcdClient.wasm.contractQuery(
    p2pContractAddress,
    {
      get_all_trades: {
  ...
  return tradeInfoResponse?.trades.map(
    (trade: any): Trade => keysToCamel(trade)
  );
}
...
async function getAllCounterTrades(
  ...
  const counterTradeInfoResponse: any = await limitPromise(() =>
    lcdClient.wasm.contractQuery(p2pContractAddress, {
      ...
      (counterTrade) =>
        new Promise(async (resolve, reject) =>
          resolve([
            counterTrade,
            keysToCamel(
              await lcdClient.wasm.contractQuery(env.contracts.p2p, {
                trade_info: {
...
async function getAssociatedAcceptedCounterTrades(
  trades: Trade[]
): Promise<any[]> {
  return Promise.all(
    trades.map(
...
async function getCancelledCounterTradesAndTrade(
  myAddress: null | string = null
): Promise<any[]> {
  let params = {
...
async function getReviewedTrades(myAddress: null | string = null) {
  ...
  return counterTrades.filter(([_, trade]: [Trade, TradeInfo]) => {
    return [TradeState.Countered, TradeState.Published]
      .map(stateToString)
...
async function getAcceptedCounterTradesToWithdraw(
  myAddress: null | string = null
) {
  let params = {
  ...
  return associatedTrades.filter(
    ([_, trade]: [Trade, any]) => !trade.assetsWithdrawn
  );
}
...
async function getAcceptedTradesToWithdraw(myAddress: null | string = null) {
  ...
  return associatedTrades.filter(
    ([_, counterTrade]: [Trade, TradeInfo]) => !counterTrade.assetsWithdrawn
  );
}
...
async function updateCounterTradeDB(db: any) {
  ...
  return Promise.all(
    counterTrades.map((counterTrade: Trade) => {
      asyncAction(
        db.set(
          `counterTradeInfo${counterTrade.tradeId}-${counterTrade.tradeId}`,
...
async function main() {
  ...
  //.catch(() =>{});
  await knex.schema.dropTable('trade_associated_assets').catch(() => {});
  await knex.schema.dropTable('whitelisted_users').catch(() => {});
  await knex.schema.dropTable('nfts_wanted').catch(() => {});
  await knex.schema
  ...
  await fetchAllTradesUntilEnd({}).then(async (trades) =>
    Promise.all(
      trades.map((trade) => {
